...
if __name__ == '__main__':
    ...
    if 1:
        ...
        key = 'open sesame'
        ot  = ('Now is the time for all good men to come to the ' + 
               'aid of their country.') # ot = original text
        print '      ot', ot
        print
    ...
    if 1:
        ...
        def test_ecb(key, clear, crypt):
            bf = Blowfish(key.decode('hex'))
            ciphertext = bf.encipher_block(clear.decode('hex'))
            plaintext  = bf.decipher_block(ciphertext)
            if (ciphertext != crypt.decode('hex') or 
                 plaintext != clear.decode('hex')):
                template = '*** failure:  key=%s, clear=%s, crypt=%s'
                print ('      %s' % template) %(key, clear, crypt)
                return 1
            return 0
        ...
        print '      %d tests failed out of %d tests performed' % (fails, tests)
        print
